...
module.exports = {
  ...
  check_login: async function (data) {
    ...
    //存在
    if (exist) {
      const result = await check_password(data.user_name, data.user_password)
  ...
  change_password: async function (data) {
    //檢查舊密碼
    const check_password_result = await check_password(data.user_name, data.user_password_old)

    if (check_password_result) {
      const result = await change_password(data.user_name, data.user_password_new)
...
function check_password(user_name, user_password) {
  return new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      ...
      const sql = `SELECT 1 FROM account WHERE user_name  = '${user_name}' AND user_password  = '${user_password}' `
...
//修改密碼
function change_password(user_name, user_password_new) {
  return new Promise((resolve, reject) => {
    pool.getConnection((err, connection) => {
      if (err) throw err
      const sql = `UPDATE account SET user_password='${user_password_new}'  WHERE user_name = '${user_name}'`
